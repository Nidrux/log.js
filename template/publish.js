...
function buildNav(members) {
  ...
  nav = nav.concat(
    buildMemberNav(members.externals, "Externals", seen, linktoExternal)
  );
  nav = nav.concat(buildMemberNav(members.events, "Events", seen, linkto));
  nav = nav.concat(
    buildMemberNav(members.namespaces, "Namespaces", seen, linkto)
  );
  nav = nav.concat(buildMemberNav(members.mixins, "Mixins", seen, linkto));
  nav = nav.concat(
    buildMemberNav(members.interfaces, "Interfaces", seen, linkto)
  );
  ...
  if (members.globals.length) {
    ...
    members.globals.forEach(function (item) {
      if (item.kind !== "typedef" && !hasOwnProp.call(seen, item.longname)) {
        nav.push(
          buildNavItem(
            buildNavType(item.kind, linkto(item.longname, item.name))
          )
        );
      }
    ...
    items.forEach(function (item) {
      ...
      if (!hasOwnProp.call(itemsSeen, item.longname)) {
        if (!!conf.default.useLongnameInNav) {
          ...
          if (
            conf.default.useLongnameInNav > 0 &&
            conf.default.useLongnameInNav !== true
          ) {
        ...
        if (methods.length) {
          methods.forEach(function (method) {
            ...
            nav.push(
              buildNavItem(
                buildNavType(method.kind, linkto(method.longname, method.name))
              )
            );
          });
        }
...
function buildNavHeading(content) {
  return ['<li class="nav-heading">', content, "</li>"].join("");
}
